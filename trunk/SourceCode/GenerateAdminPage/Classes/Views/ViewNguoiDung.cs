...
namespace GenerateAdminPage.Classes
{
    public class ViewNguoiDung: Views
    {
        ...
        public string GenerateTemplateList()
        {
            ...
            Result += TAB3 + "</td>" + END;
            Result += TAB3 + "<td>" + END;
            Result += TAB4 + "<%= Ajax.ActionLink(\"Delete\", \"Delete" + GlobalVariables.g_ModelName + "\", \"Admin\", new { id = Model." + GlobalVariables.g_ModelName + "Model.GetModel.LstObjModel[i].BaseInfo.ID }, new AjaxOptions{Confirm = \"Are you sure you want to Delete it? This action cannot be undone.\", HttpMethod = \"Delete\", OnComplete = \"jsonDelete_OnComplete\"})%>" + END;
            Result += TAB3 + "</td>" + END;
            Result += TAB2 + "</tr>" + END;
            Result += TAB + "<% } %>  " + END;
            Result += "</table>" + END;
